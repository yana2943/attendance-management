...
$startTime   = old('start_time', $attendance->start_time);
$endTime     = old('end_time', $attendance->end_time);
$break1Start = old('break_start', $attendance->break_start);
$break1End   = old('break_end', $attendance->break_end);
$break2Start = old('break2_start', $attendance->break2_start);
$break2End   = old('break2_end', $attendance->break2_end);
...
<div class="attendance-detail">
    <form action="{{ route('admin.attendance.update') }}" method="POST">
        ...
        <div class="form-error">@error('start_time') {{ $message }} @enderror</div>
        <div class="form-error">@error('end_time') {{ $message }} @enderror</div>
        <div class="border"></div>
        ...
        <div class="form-error">@error('break_start') {{ $message }} @enderror</div>
        <div class="form-error">@error('break_end') {{ $message }} @enderror</div>
        <div class="border"></div>
        ...
        <div class="form-error">@error('break2_start') {{ $message }} @enderror</div>
        <div class="form-error">@error('break2_end') {{ $message }} @enderror</div>
        <div class="border"></div>

        <div class="detail-item">
            <span class="label">備考</span>
            <textarea name="note" rows="3" @if($isPending) disabled @endif>{{ $note }}</textarea>
        </div>
        <div class="form-error">@error('note') {{ $message }} @enderror</div>
        <div class="border"></div>
